...
EnemyMoveSinusoidNarrow::EnemyMoveSinusoidNarrow(Game *game, olc::vf2d position, std::string spriteLocation) :
    Enemy(game, position, spriteLocation) {}

void EnemyMoveSinusoidNarrow::Movement(float fElapsedTime)
{
    data[0] += fElapsedTime;
    position.y += 0.5f * game->scrollSpeed * fElapsedTime;
    position.x += 50.0f * cosf(data[0]) * fElapsedTime;
}

void EnemyMoveSinusoidNarrow::Firing(float fElapsedTime)
{
